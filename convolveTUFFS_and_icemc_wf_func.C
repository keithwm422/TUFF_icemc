/* Per Linda,
This is a function that will be included in anita.cc (called readTuffResponseDigitizer)
and this program is for testing the function. 

The function will read Andrew's responses from the data directory in icemc 
(maybe have a subdirectory for all the subdirectories that house the 96 files for the notches config).

Add call to this new function in anita.cc (around readImpulseResponseDigitizer)

In ChanTrigger.C look for applyImpulseResponse around the line that says  //Calculate convolution
  TGraph *surfSignal = FFTtools::getConvolution(graphUp, anita1->fSignalChainResponseDigitizer[ipol][iring][iphi]);
and "add some if statements of the type"
"
  if  we want to use the TUFF responses: 
   check what's the realTime, load the right impulse response
 convolve the right TUFF response with graphUp
"
...
Important Checks:
Is the response loaded?
Is the correct response loaded?
Check the real time compared to something else?
Check the convolved waveform with original response.
